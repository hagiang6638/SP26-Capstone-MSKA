...
class Recognition(nn.Module):
    def __init__(self, cfg, args, input_streams=None):
        ...
        # Đóng băng backbone nếu được chỉ định trong file config (dùng khi finetune dữ liệu nhỏ)
        if self.cfg.get('freeze_backbone', False) and self.input_type == 'keypoint':
            for param in self.visual_backbone_keypoint.parameters():
                param.requires_grad = False
            print("[INFO] ĐÃ ĐÓNG BĂNG (FREEZE) BACKBONE DSTA-NET. Chỉ train Visual Heads.")

    def train(self, mode=True):
        """
        Overwrite hàm train để đảm bảo backbone luôn ở trạng thái eval() 
        khi freeze (ngăn Dropout và BatchNorm cập nhật).
        """
        super(Recognition, self).train(mode)
        if self.cfg.get('freeze_backbone', False) and self.input_type == 'keypoint':
            self.visual_backbone_keypoint.eval()
        return self
