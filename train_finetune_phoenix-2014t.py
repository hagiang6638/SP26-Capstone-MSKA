...
def get_args_parser():
    ...
    parser.add_argument("--wandb_id", type=str, default='',
                        help="wandb run id to resume (e.g. for Google Colab runtimes)",
                        )
...
def main(args, config):
    ...
    if args.finetune:
        ...
        # =========================
        # 1. REMOVE gloss_output_layer (ONLY if shapes mismatch)
        # =========================
        remove_keys = []
        for k in list(state_dict.keys()):
            if 'gloss_output_layer' in k:
                if k not in model_state or state_dict[k].shape != model_state[k].shape:
                    remove_keys.append(k)

        for k in remove_keys:
            del state_dict[k]

        print(f"[INFO] Removed {len(remove_keys)} gloss_output_layer params (shape mismatch)")

        # =========================
        # 2. REMOVE old BN (bn1.norm) (ONLY if shapes mismatch)
        # =========================
        remove_keys = []
        for k in list(state_dict.keys()):
            if 'bn1.norm' in k:
        ...
        print(f"[INFO] Removed {len(remove_keys)} old BN params (shape mismatch)")
    ...
    with torch.no_grad():
        ...
        if do_recognition:
            ...
            for hyp_name in results[name].keys():
                ...
                if not any(len(r.strip()) > 0 for r in gls_ref):
                    evaluation_results[k + 'wer_list'] = {'wer': 200}
                    continue
...
def setup_run(args, config):
    def do_init(group=None):
        ...
        # Lấy ID trực tiếp từ tham số người dùng nhập vào trên Colab
        if args.wandb_id:
    ...
    if args.log_all:
        ...
        out_dir = getattr(args, 'output_dir', config['training']['model_dir'])
        run = do_init(group=out_dir.split('/')[-1])
